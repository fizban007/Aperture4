...
void damping_boundary(vector_field<Conf> &E, vector_field<Conf> &B,
  ...
  kernel_launch(
      [damping_length, damping_coef] __device__(auto e, auto b, auto e0, auto b0) {
        auto &grid = dev_grid<Conf::dim>();
        auto ext = grid.extent();
...
void field_solver_gr_ks_cu<Conf>::update_Bth(vector_field<Conf> &B,
  ...
  auto Bth_kernel = [dt, a, beta,
                     is_boundary] __device__(auto D, auto B1_0, auto B1_1,
                                             auto result, auto grid_ptrs) {
    using namespace Metric_KS;
...
void field_solver_gr_ks_cu<Conf>::update_Bph(vector_field<Conf> &B,
  ...
  auto Bph_kernel = [dt, a, beta,
                     is_boundary] __device__(auto D, auto B2_0, auto B2_1,
                                             auto result, auto grid_ptrs) {
    using namespace Metric_KS;
    auto &grid = dev_grid<Conf::dim>();
    auto ext = grid.extent();
...
void field_solver_gr_ks_cu<Conf>::update(double dt, uint32_t step) {
  ...
  if (this->m_comm == nullptr || this->m_comm->domain_info().is_boundary[1]) {
    damping_boundary(*(this->E), *(this->B), *(this->E0), *(this->B0), m_damping_length, m_damping_coef);
  }
