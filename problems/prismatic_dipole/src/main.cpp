...
int main(int argc, char* argv[]) {
  ...
  auto updater = env.register_system<prismatic_ptc_updater>(
      mesh, solver->E_e(), solver->B_f(), solver->J_e());

  env.register_system<prismatic_sph_output>(
      mesh, solver->E_e(), solver->B_f(),
      &solver->J_e(), &updater->rho());

  env.init();

  // Inject particles from config (optional)
  ...
  if (n_inject > 0) {
    ...
    Scalar px = ptc_pr * r_hat_x;
    Scalar py = ptc_pr * r_hat_y;
    Scalar pz = ptc_pr * r_hat_z;

    for (int i = 0; i < n_inject; i++) {
      updater->add_particle(x, y, z, px, py, pz, ptc_weight,
