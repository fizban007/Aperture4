...
void
inject_particles(particle_data_t& ptc, rng_states_t<exec_tags::device>& rng_states,
...
boundary_condition<Conf>::update(double dt, uint32_t step) {
  ...
  if (step % 1 == 0 && time > m_tp_start && time < m_tp_end) {
    inject_particles<Conf>(*ptc, *rng_states, m_surface_n, 10, m_weight, m_grid, m_rpert1, m_rpert2);
  }
}
