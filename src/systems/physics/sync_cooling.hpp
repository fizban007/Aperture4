...
HD_INLINE void
sync_cooling(ptc_float_t& p1, ptc_float_t& p2, ptc_float_t& p3,
             ptc_float_t& gamma, value_t B1, value_t B2, value_t B3, value_t E1,
             value_t E2, value_t E3, value_t q_over_m, value_t cooling_coef,
             value_t B0) {
...
template <typename ptc_float_t, typename value_t>
HD_INLINE void
sync_kill_perp(ptc_float_t& p1, ptc_float_t& p2, ptc_float_t& p3,
               ptc_float_t& gamma, value_t E1, value_t E2, value_t E3,
               value_t B1, value_t B2, value_t B3, value_t q_over_m,
               value_t cooling_coef, value_t B0) {
  // B1 /= q_over_m;
  // B2 /= q_over_m;
  // B3 /= q_over_m;
  ...
  value_t delta_p1 = -cooling_coef * (p1 - B1 * pdotB / B_sqr);
  value_t delta_p2 = -cooling_coef * (p2 - B2 * pdotB / B_sqr);
  value_t delta_p3 = -cooling_coef * (p3 - B3 * pdotB / B_sqr);
...
template <typename ptc_float_t, typename value_t>
HD_INLINE void
sync_kill_gyration(ptc_float_t& p1, ptc_float_t& p2, ptc_float_t& p3,
                   ptc_float_t& gamma, value_t E1, value_t E2, value_t E3,
                   value_t B1, value_t B2, value_t B3, value_t q_over_m,
                   value_t cooling_coef, value_t B0) {
  ...
  if (vd_sqr > 1.0e-5) {
    vd1 *=
        (1.0f - math::sqrt(max(1.0f - 4.0f * vd_sqr, 0.0f))) / (2.0f * vd_sqr);
    vd2 *=
        (1.0f - math::sqrt(max(1.0f - 4.0f * vd_sqr, 0.0f))) / (2.0f * vd_sqr);
    vd3 *=
        (1.0f - math::sqrt(max(1.0f - 4.0f * vd_sqr, 0.0f))) / (2.0f * vd_sqr);
  ...
  value_t delta_v1 = -cooling_coef * (p1 - B1 * vdotB / B_sqr - gamma * vd1);
  value_t delta_v2 = -cooling_coef * (p2 - B2 * vdotB / B_sqr - gamma * vd2);
  value_t delta_v3 = -cooling_coef * (p3 - B3 * vdotB / B_sqr - gamma * vd3);
