...
boundary_condition<Conf>::update(double dt, uint32_t step) {
  ...
  kernel_launch([ext, time, omega] __device__ (auto e, auto b, auto e0, auto b0) {
      ...
      for (auto n : grid_stride_range(0, ext.size())) {
        ...
        value_t r = grid_sph_t<Conf>::radius(grid.template coord<0>(pos[0], false));
        value_t r_s = grid_sph_t<Conf>::radius(grid.template coord<0>(pos[0], true));
        value_t r_reduced = grid_sph_t<Conf>::radius(grid.template coord<0>(pos[0] - 1, false));
        // value_t theta = grid_sph_t<Conf>::radius(grid.template coord<1>(pos[1], false));
        // value_t theta_s = grid_sph_t<Conf>::radius(grid.template coord<1>(pos[1], true));

        if (r < 1.0f) {
          value_t theta_s = grid_sph_t<Conf>::theta(grid.template coord<1>(pos[1], true));
        ...
        if (r_reduced < 1.0f) {
          value_t theta = grid_sph_t<Conf>::theta(grid.template coord<1>(pos[1], false));
          b[0][idx] = 0.0f;
      ...
      //   value_t theta = grid_sph_t<Conf>::theta(grid.template coord<1>(n1, false));
      //   value_t theta_s = grid_sph_t<Conf>::theta(grid.template coord<1>(n1, true));

      //   // For quantities that are not continuous across the surface
      //   for (int n0 = 0; n0 < grid.guard[0] + 4; n0++) {
      //     auto idx = idx_t(index_t<2>(n0, n1), ext);
      //     value_t r = grid_sph_t<Conf>::radius(grid.template coord<0>(n0, false));
      //     e[0][idx] = omega * sin(theta_s) * r * b0[1][idx];
      //     b[1][idx] = 0.0;
      //     b[2][idx] = 0.0;
      //   }
      //   // For quantities that are continuous across the surface
      //   for (int n0 = 0; n0 < grid.guard[0] + 5; n0++) {
      //     auto idx = idx_t(index_t<2>(n0, n1), ext);
      //     value_t r_s = grid_sph_t<Conf>::radius(grid.template coord<0>(n0, true));
      //     b[0][idx] = 0.0;
      //     e[1][idx] = -omega * sin(theta) * r_s * b0[0][idx];
      //     e[2][idx] = 0.0;
      //   }
