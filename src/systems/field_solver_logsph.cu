...
//                 gp.Ab[1][idx];
//             if (pos[0] == 3 && pos[1] == 209) {
//               printf("Btheta is %f, Ephi is %f, %f\n", result[1][idx],
//                      e[2][idx], e[2][idx_px]
//                      // circ1(e, gp.le, idx, idx_px),
//                      // circ1(e0, gp.le, idx, idx_px)
//               );
      ...
      [dt, ext] __device__(auto result, auto e, auto gp) {
        ...
        for (auto n : grid_stride_range(0, ext.size())) {
          ...
          if (grid.is_in_bound(pos)) {
            ...
            // if (pos[0] == 3 && pos[1] == 256) {
            //   printf("From B update, Btheta is %f, Ephi is %f, %f, %f\n",
            //          result[1][idx], e[2][idx.dec_x()],
            //          e[2][idx], e[2][idx_px]);
            // }
...
//   typedef typename Conf::value_t value_t;
//   kernel_launch(
//       [ext, depth] __device__(auto e, auto e0) {
//         auto& grid = dev_grid<Conf::dim>();
//         for (auto n1 : grid_stride_range(0, grid.dims[1])) {
...
damping_boundary(vector_field<Conf>& e, vector_field<Conf>& b,
  ...
  typedef typename Conf::value_t value_t;
  kernel_launch(
      [ext, damping_length, damping_coef] __device__(auto e, auto b, auto e0,
                                                     auto b0) {
        auto& grid = dev_grid<Conf::dim>();
        for (auto n1 : grid_stride_range(0, grid.dims[1])) {
