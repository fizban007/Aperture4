...
HOST_DEVICE Scalar double_ffe_sheet_Bz(Scalar B0, Scalar y, Scalar ysize, Scalar delta, Scalar b_g) {
  if (y < 0.0f) {
    return B0 * sqrt(sech((y + 0.25f * ysize) / delta)**2 + b_g**2);
  } else {
    return B0 * sqrt(sech((y - 0.25f * ysize) / delta)**2 + b_g**2);
  }
}
...
HOST_DEVICE Scalar drift_frac(Scalar yarg, Scalar b_g) {
    return sech(yarg) * sqrt((1.0f + b_g*b_g) / (1.0f + square(b_g * std::cosh(yarg))));
}
...
double_ffe_current_sheet(vector_field<Conf> &B, particle_data_t &ptc,
  ...
  injector.inject_pairs(
      ...
      [n_upstream, q_e, delta, ysize, B_g] LAMBDA(auto &x_global, PtcType type) {
        ...
        return 1.0 / q_e / n_upstream * (1.0f - drift_frac(yarg, B_g));
      });
