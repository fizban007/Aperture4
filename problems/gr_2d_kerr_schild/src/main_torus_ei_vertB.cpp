...
namespace Aperture {
  ...
  HOST_DEVICE vec_t<Scalar,3> torus_momentum(Scalar r, Scalar th, Scalar a, Scalar Emax, Scalar Temp, Scalar Lz, rand_state& state) {
    ...
    // Sampling. Unlike torus_vertB, which draws phat^2 from the
    // non-relativistic exp(-phat^2/2That), this inverts the CDF of the exact
    // relativistic Boltzmann factor exp(-(ghat-1)/That) in the Lorentz factor
    // ghat = sqrt(1+phat^2), truncated at ghat_max. Since ghat - 1 =
    // (E - Emin)/(alpha*C), this is consistent with the energy-space Boltzmann
    // integral that torus_Density uses to set the particle weights. The two
    // agree to O(phat^2); they diverge once That approaches ~0.1.
...
//   exec_policy_dynamic<Conf>::sync();
  ...
  // The initial torus is an electron-ion plasma rather than pairs. Slots are
  // still filled two at a time -- even slot electron, odd slot PtcType::ion --
  // so the torus starts charge neutral; the ion/electron mass ratio is the
  // "ion_mass" parameter. Secondaries from pair production remain e+/e- with
  // mass ratio 1, since only PtcType::ion is scaled by ion_mass.
  // NOTE: requires num_species = 3 in the config, because PtcType::ion == 2
  // and the ion charge deposit indexes Rho[sp] with no bounds check.
  ptc_inj.inject_e_i_plasma(
