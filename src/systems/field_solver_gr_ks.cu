...
void field_solver_gr_ks_cu<Conf>::update_Bph(vector_field<Conf> &B,
    ...
    for (auto idx : grid_stride_range(Conf::begin(ext), Conf::end(ext))) {
      ...
      if (grid.is_in_bound(pos)) {
        ...
        if (pos[0] == 8 && pos[1] == 250) {
          printf("Er0 is %f, Er1 is %f, Eth0 is %f, Eth1 is %f, dBphi is %f\n",
                 Er0, Er1, Eth0, Eth1,
  ...
  kernel_launch(Bph_kernel, D.get_const_ptrs(), m_tmp_prev_field.dev_ndptr_const(),
                m_tmp_prev_field.dev_ndptr_const(), m_tmp_predictor.dev_ndptr(), m_a,
                m_ks_grid.get_grid_ptrs());
  CudaSafeCall(cudaDeviceSynchronize());
