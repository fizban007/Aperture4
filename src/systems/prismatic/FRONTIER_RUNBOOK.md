...
| 15 | **dt-denominated GCA/Boris switch** (pitfall #14's twin) | (historical) the hybrid switch tested `ω_c·dt/γ > 0.1`, so the PHYSICAL switching surface moved with the step: the same "0.1" meant rate 10.2 at L5, 20.4 at L6, 40.7 at L7. At L7 the Boris region covered 96.5% of the LC annulus vs L6's 80%, giving Y-point jitter 21.3° (L6: 12.0), +22% open flux, ~×1.5 luminosity excess, and a spurious P/2 shedding cycle. The `gca005` A/B branch (0.05 at L7 dt ≡ rate 20.37 ≡ L6's 0.1) restored the L6 surface — Boris fraction 0.795 vs 0.80 — confirming the diagnosis. | CLOSED: the criterion is now the RATE `ω_c/γ > gca_switch_omegac` with no dt in it; `gca_switch_omegac_dt` aborts at init printing the per-config conversion (`old/dt`). Production value 20.0 = the L6 baseline surface, now shared by every level. Regression test `"Hybrid switch dispatch is dt-invariant"` (`tests/test_prismatic_pusher.cpp`) requires bit-identical per-particle dispatch at dt, dt/2, dt/4. **New obligation the rate form introduces:** `gca_switch_omegac·dt` is the rad/step Boris sees at the switch — the old units capped it implicitly, the rate form does not, so init warns above 0.25 and aborts above 0.5 (~12 steps/gyration). At 20.0: 0.196 rad/step at L5, 0.098 at L6, 0.049 at L7. |
| 16 | **Cooling welded to the integrator boundary** | (historical) GCA held μ = 0 (synchrotron-locked) while Boris had NO radiative drag at all, so the two sides of the switch ran different physics and the switch position — itself dt-dependent, #15 — had a physical footprint. | CLOSED: Landau-Lifshitz drag on the Boris branch (`src/systems/physics/radiation_reaction.hpp`, shared with `pusher_synchrotron`), operator-split after `boris_push`; GCA path unchanged. Knob is `sync_cooling_coef` (or `sync_gamma_rad` anchored at `sync_cool_b_lc`), a rate coefficient in inverse time units. `t_cool(γ,B) = 1/(c_r γ B²)`. **Two things not to re-litigate:** (a) do NOT damp the drift-frame perpendicular momentum instead — that drives the pitch angle to zero at every γ, whereas real synchrotron drag at high γ is anti-parallel to **u** and preserves it; the LL form also needs no `E ≥ B` or `B → 0` guard since it falls smoothly to zero with the field, leaving the sheet's hot core uncooled for free. (b) the drag is integrated with an EXPONENTIAL midpoint, not a plain implicit-midpoint fixed point — the latter stops contracting at `ν·dt ≳ 1` and diverges to NaN, which is exactly the locked regime the runs target. |
| 17 | **Adiabaticity as a switch criterion — REJECTED, do not re-propose** | An early design replaced the switch with `ρ_eff·κ < ε_ad` from the recovery gradient. In an unstable current sheet this is untrustworthy: `ρ_eff = γm/(\|q\|B)` diverges exactly where `B → 0`, `κ` needs `b = B/\|B\|` which is pure noise there, and the recovery gradient is piecewise-constant per prism (first order). It is a ratio of two ill-conditioned quantities precisely where the interesting physics is, and the noise would land on the switch decision with a momentum-representation conversion per flip. | NOT IMPLEMENTED, by decision. The rate criterion (#15) depends only on \|B\| and γ, both well-conditioned. Known accepted gap: a pure rate criterion cannot see the Y-point/separatrix, where B is moderate but the curvature radius is small, and sends those particles to GCA where GCA is invalid. If that ever needs fixing, add adiabaticity as an OPTIONAL, ONE-DIRECTIONAL refinement (may only move particles GCA→Boris, never the reverse), gated to where B is large enough that `b` is meaningful, and never allowed to override the resolution guard in #15. |
