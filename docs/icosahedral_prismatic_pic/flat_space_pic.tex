...
\paragraph{The circumcenter of a triangle.}
Every triangle has a unique \emph{circumcenter}: the point equidistant from all
three vertices, or equivalently, the center of the circle passing through all
three vertices (the \emph{circumscribed circle}). Geometrically, it lies at the
intersection of the three perpendicular bisectors of the triangle's edges. For
an equilateral triangle, the circumcenter coincides with the centroid (center of
mass), but for a general triangle they differ.

Given triangle vertices $\bm{p}_0, \bm{p}_1, \bm{p}_2$ in $\mathbb{R}^3$, the
circumcenter is $\bm{c} = \bm{p}_0 + s\,\bm{d}_{10} + t\,\bm{d}_{20}$ where
$\bm{d}_{10} = \bm{p}_1 - \bm{p}_0$, $\bm{d}_{20} = \bm{p}_2 - \bm{p}_0$,
and $(s, t)$ solve
\begin{equation}
  \begin{pmatrix}
    2\,|\bm{d}_{10}|^2 & 2\,\bm{d}_{10}\cdot\bm{d}_{20} \\
    2\,\bm{d}_{10}\cdot\bm{d}_{20} & 2\,|\bm{d}_{20}|^2
  \end{pmatrix}
  \begin{pmatrix} s \\ t \end{pmatrix}
  =
  \begin{pmatrix} |\bm{d}_{10}|^2 \\ |\bm{d}_{20}|^2 \end{pmatrix}.
  \label{eq:circumcenter}
\end{equation}
This follows from requiring $|\bm{c} - \bm{p}_0|^2 = |\bm{c} - \bm{p}_1|^2 =
|\bm{c} - \bm{p}_2|^2$. For our icosahedral triangles on the unit sphere, the
circumcenter is computed from the flat triangle in $\mathbb{R}^3$ and then
projected back onto the sphere by normalizing.

\paragraph{Circumcentric (Voronoi) dual mesh.}
For each prism in the mesh, a \emph{dual vertex} is placed at the circumcenter
of its angular triangle, at the radial midpoint of the layer. The key property
of the circumcenter is that it lies on the perpendicular bisector of every edge
of the triangle. Therefore, the line connecting the circumcenters of two
adjacent triangles (sharing an edge) is perpendicular to that shared edge. This
perpendicularity is the defining property of the \emph{Voronoi dual} (also
known as the circumcentric dual).

The dual face of a primal edge $e$ is the polygon formed by connecting the dual
vertices of all prisms sharing $e$. The dual edge of a primal face $f$ connects
the dual vertices of the two prisms sharing $f$. The diagonal Hodge star is:
\begin{equation}
...
\paragraph{Why circumcenters matter.}
The diagonal Hodge approximates the constitutive relation by assuming that the
field component along a primal edge is proportional to the flux through the
dual face. This is exact when the dual face is perpendicular to the edge,
because then both measure the same component of the field. The circumcentric
dual guarantees this perpendicularity by construction; using centroids (centers
of mass) instead of circumcenters does not.

Empirically, using centroids leads to $\sim 20$--$60\%$ energy drift in a
static dipole field test over $\sim 10$ light-crossing times, while
circumcenters reduce the drift to $\sim 2$--$3\%$. This dramatic improvement
is entirely due to the edge--dual-face orthogonality.
