...
// 3D Kerr-Schild torus test.
//
// This is a 3D port of the 2D torus_ei setup, configured as a clean test:
//   - zero initial electric and magnetic fields (the B-field loop below is
//     left commented out, so B0/E0/Bdelta/Edelta all stay at their zero
//     defaults),
//   - electron-positron plasma (inject_pairs),
//   - intended for a relatively short evolution (set via max_steps in the
//     config).
//
// All of the torus physics is axisymmetric and depends only on (r, theta), so
// the helper functions and injection lambdas carry over from the 2D case
// unchanged; only the grid configuration (Config<3>) and the extra azimuthal
// dimension differ.
...
main(int argc, char  * argv[]) {
  ...
  // Radiation is disabled for this test.
  ...
  // Zero initial fields: leave B0/E0/Bdelta/Edelta at their zero defaults by
  // not initializing them here. The pointers are fetched for completeness and
  // in case the magnetic-field loop below is re-enabled.
...
// magnetic field loop (disabled for the zero-field test). To give the torus an
// initial poloidal field, uncomment this block. Note: the gradients here are
// taken in (r, theta) only, consistent with an axisymmetric vector potential.
...
//   exec_policy_dynamic<Conf>::sync();
  ...
  ptc_inj.inject_pairs(
      ...
      // cells that inject and false for cells that do nothing. The torus is
      // axisymmetric, so this depends only on (r, theta).
