...
#include "field_solver_gr_ks_mod.h"
...
template <typename Conf, template <class> class ExecPolicy>
field_solver_mod<Conf, ExecPolicy, coord_policy_gr_ks_sph>::field_solver_mod(
...
template <typename Conf, template <class> class ExecPolicy>
void
field_solver_mod<Conf, ExecPolicy, coord_policy_gr_ks_sph>::init() {
...
template <typename Conf, template <class> class ExecPolicy>
void
field_solver_mod<Conf, ExecPolicy, coord_policy_gr_ks_sph>::compute_aux_E(
        ...
        ExecPolicy<
          ...
          if (pos[0] > 0 && pos[0] < grid.dims[0] - 1) {
            ...
            if (math::abs(th) < 0.1f * grid.delta[1] ||
              ...
              auxE[0][idx] =
                  r * grid.delta[0] * Metric_KS::ag_11(a, r, th) * D[0][idx];
              ...
              w_minus = Metric_KS::sqrt_gamma(a, r_minus, th);
              w_plus = Metric_KS::sqrt_gamma(a, r_plus, th);
              auxE[0][idx] =
                  r * grid.delta[0] * Metric_KS::alpha(a, r, th) * (
                      Metric_KS::g_11(a, r, th) * (
                          D[0][idx]
                      ) +
                      Metric_KS::g_13(a, r, th) * (
                          (w_minus * D[2][idx] + w_plus * D[2][idx.inc_x()]) /
                          (w_minus + w_plus)
                      )
                  );
              // auxE[0][idx] =
            ...
            auxE[1][idx] =
                grid.delta[1] * (
                    Metric_KS::alpha(a, r, th) * Metric_KS::g_22(a, r, th) * (
                        D[1][idx]
                    ) -
                    Metric_KS::sqrt_gamma(a, r, th) * Metric_KS::beta1(a, r, th) * (
                        (w_minus * B[2][idx.dec_x()] + w_plus * B[2][idx]) /
                        (w_minus + w_plus)
                    )  
                );
            // auxE[1][idx] =
            ...
            if (math::abs(th) < 0.1f * grid.delta[1] ||
                math::abs(th - M_PI) < 0.1f * grid.delta[1]) {
              auxE[2][idx] =
                  Metric_KS::alpha(a, r, th) * Metric_KS::g_13(a, r, th) * (
                      (w_minus * D[0][idx.dec_x()] + w_plus * D[0][idx]) / 
                      (w_minus + w_plus)
                  );
              // auxE[2][idx] =
              ...
              auxE[2][idx] =
                  ...
                  Metric_KS::sqrt_gamma(a, r, th) * Metric_KS::beta1(a, r, th) * (
                      (w_minus * B[1][idx.dec_x()] + w_plus * B[1][idx]) / 
                      (w_minus + w_plus)
                  );
              // auxE[2][idx] =
...
template <typename Conf, template <class> class ExecPolicy>
void
field_solver_mod<Conf, ExecPolicy, coord_policy_gr_ks_sph>::compute_aux_H(
    const vector_field<Conf>& B, const vector_field<Conf>& D) {
  auto a = m_a;
  m_auxH->init();
        ...
        ExecPolicy<Conf>::loop(
            Conf::begin(ext), Conf::end(ext), [&] LAMBDA(auto idx) {
              ...
              if (pos[0] > 0 && pos[0] < grid.dims[0] - 1) {
                ...
                value_t w_minus = Metric_KS::sqrt_gamma(a, r_minus, th);
                value_t w_plus = Metric_KS::sqrt_gamma(a, r_plus, th);
                ...
                // H_3 = alpha B_3                           - sqrt(gam) beta^1 D^2
                //     = alpha (gamma_31 B^1 + gamma_33 B^3) - sqrt(gam) beta^1 D^2
                auxH[2][idx] =
                    Metric_KS::alpha(a, r, th) * (
                        Metric_KS::g_33(a, r, th) * (
                            B[2][idx]
                        ...
                        Metric_KS::g_13(a, r, th) * (
                            (w_minus * B[0][idx] + w_plus * B[0][idx.inc_x()]) /
                            (w_minus + w_plus)
                        )
                    ) -
                    Metric_KS::sqrt_gamma(a, r, th) * Metric_KS::beta1(a, r, th) * (
                        (w_minus * D[1][idx] + w_plus * D[1][idx.inc_x()]) /
                        (w_minus + w_plus)
                    );
                // auxH[2][idx] =
...
template <typename Conf, template <class> class ExecPolicy>
void
field_solver_mod<Conf, ExecPolicy, coord_policy_gr_ks_sph>::compute_dB_dt(
...
template <typename Conf, template <class> class ExecPolicy>
void
field_solver_mod<Conf, ExecPolicy, coord_policy_gr_ks_sph>::compute_dD_dt(
...
template <typename Conf, template <class> class ExecPolicy>
void
field_solver_mod<Conf, ExecPolicy, coord_policy_gr_ks_sph>::iterate_predictor(
    double dt) {
...
template <typename Conf, template <class> class ExecPolicy>
void
field_solver_mod<Conf, ExecPolicy, coord_policy_gr_ks_sph>::update_semi_implicit(
...
template <typename Conf, template <class> class ExecPolicy>
void
field_solver_mod<Conf, ExecPolicy, coord_policy_gr_ks_sph>::update_explicit(
...
template <typename Conf, template <class> class ExecPolicy>
void
field_solver_mod<Conf, ExecPolicy, coord_policy_gr_ks_sph>::compute_divs_e_b() {
  this->divE->init();
  this->divB->init();
  auto a = m_a;
  // if (this->m_comm != nullptr) {
...
template <typename Conf, template <class> class ExecPolicy>
void
field_solver_mod<Conf, ExecPolicy, coord_policy_gr_ks_sph>::compute_EB_sqr() {}

template <typename Conf, template <class> class ExecPolicy>
void
field_solver_mod<Conf, ExecPolicy, coord_policy_gr_ks_sph>::compute_DdotB_J_B() {
  DdotB->init();
  Bmag->init();
  Jmag->init();
  auto a = m_a;
