...
#ifndef FIELD_SOLVER_CARTESIAN_IMPL_H_
#define FIELD_SOLVER_CARTESIAN_IMPL_H_
...
field_solver<Conf, ExecPolicy, coord_policy_cartesian>::compute_e_update_pml(
  ...
  ExecPolicy<Conf>::launch(
      [dt, pml_len, damping] LAMBDA(auto result, auto e1, auto e2, auto b,
        ...
        ExecPolicy<Conf>::loop(Conf::begin(ext), Conf::end(ext),
                               [&] LAMBDA (auto idx) {
          ...
          if (grid.is_in_bound(pos)) {
            ...
            if (damping[0] && pos[0] < pml_len + grid.guard[0]) {
              // value_t x = (pml_len + grid.guard[0] - pos[0]) * grid.delta[0];
              ...
              // value_t x = (pos[0] - (grid.guard[0] + grid.N[0] - pml_len)) *
              //             grid.delta[0];
              value_t x =
                  grid.coord(0, pos[0], true) -
            ...
            if (Conf::dim > 1 && damping[2] &&
              ...
              // value_t y = (pml_len + grid.guard[1] - pos[1]) * grid.delta[1];
              ...
              // value_t y = (pos[1] - (grid.guard[1] + grid.N[1] - pml_len)) *
              //             grid.delta[1];
              value_t y =
                  grid.coord(1, pos[1], true) -
            ...
            if (Conf::dim > 2 && damping[4] &&
              ...
              // value_t z = (pml_len + grid.guard[2] - pos[2]) * grid.delta[2];
              ...
              // value_t z = (pos[2] - (grid.guard[2] + grid.N[2] - pml_len)) *
              //             grid.delta[2];
              value_t z =
                  grid.coord(2, pos[2], true) -
...
#endif  // FIELD_SOLVER_CARTESIAN_IMPL_H_
