...
struct resonant_scattering_scheme{
  ...
  HOST_DEVICE size_t emit_photon(const Grid<Conf::dim, value_t> &grid,
    ...
    // value_t gamma_para = gamma; // This is the gamma associated with the frame of the center of gyration so I think should be the same
    // }
  ...
  // and finally convert back to the new p_para, p_mu definition
    ...
    // incident photon energy to compute the change in momentum
      value_t Eph_inc = math::abs(1/(gamma_para * (1.0f + beta_para * mu)) * // Only difference is u->mu since radial photon as angle mu
      (1.0f - 1.0f / math::sqrt(1.0f + 2.0f * b)));
      // incident photon has khat = {1,0,0} in the lab frame
      //outgoing photon has khat = {n_ph1, n_ph2, n_ph3} in the lab frame
      value_t del_p1 = Eph_inc - Eph * n_ph1;
      value_t del_p2 = -Eph * n_ph2;
      value_t del_p3 = -Eph * n_ph3;
      value_t del_p_para = del_p1 * n1 + del_p2 * n2 + del_p3 * n3; // change in p_para
      value_t del_p_perp_square =(del_p1-del_p_para*n1)*(del_p1-del_p_para*n1)+
                                 (del_p2-del_p_para*n2)*(del_p2-del_p_para*n2)+
                                 (del_p3-del_p_para*n3)*(del_p3-del_p_para*n3);
      ...
      //if (Nph > 1.0f || rng_uniform<float>(state) < Nph) {
        ...
        if (N_pois >= 1) {
          ...
          // }
          p_para += del_p_para; // change p_para appropriately
          moment_mu += del_p_perp_square / (2.0f * B_mag); // change mu appropriately
          p_perp = math::sqrt(moment_mu * B_mag * 2); // recompute p_perp
          ptc.p1[tid] = p_para; // Remember to not get confused by ptc.p1=p_para 
          ptc.p2[tid] = moment_mu;
