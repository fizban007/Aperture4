...
grid_ks_t<Conf>::compute_coef() {
  ...
  for (auto idx : m_Ab[0].indices()) {
    ...
    if constexpr (Conf::dim == 2) {
      ...
      m_Ab[0][idx] =
          gauss_quad([this, r_s](auto x) { return sqrt_gamma(a, r_s, x); },
      ...
      m_Ab[1][idx] =
          gauss_quad([this, th_s](auto x) { return sqrt_gamma(a, x, th_s); },
      ...
      m_Ab[2][idx] = gauss_quad(
          [this, r_s, pos](auto x) {
            return gauss_quad(
                [this, x](auto y) { return sqrt_gamma(a, y, x); }, r_s,
      ...
      double ph = phi(this->template coord<2>(pos[2], false));
      double ph_s = phi(this->template coord<2>(pos[2], true));

      double dth = (theta(this->template coord<1>(pos[1] + 1, true)) - th_s);
      double dr = (radius(this->template coord<0>(pos[0] + 1, true)) - r_s);
      double dph = (phi(this->template coord<2>(pos[2] + 1, true)) - ph_s);

      // B1 is staggered in r, not in theta
      m_Ab[0][idx] = sqrt_gamma(a, r_s, th) * dth * dph;
      if (m_Ab[0][idx] != m_Ab[0][idx]) {
        Logger::print_err("m_Ab0 at ({}, {}) is NaN!", pos[0], pos[1]);
      }

      // B2 is staggered in theta, not in r
      m_Ab[1][idx] = sqrt_gamma(a, r, th_s) * dr * dph;
      if (m_Ab[1][idx] != m_Ab[1][idx]) {
        Logger::print_err("m_Ab1 at ({}, {}) is NaN!", pos[0], pos[1]);
      }

      // B3 is unstaggered in either r or theta
      m_Ab[2][idx] = sqrt_gamma(a, r, th) * dr * dth;
      if (m_Ab[2][idx] != m_Ab[2][idx]) {
        Logger::print_err("m_Ab2 at ({}, {}) is NaN!", pos[0], pos[1]);
      }

      // D-components live on cell edges and thus need different deltas
      dr = r - radius(this->template coord<0>(pos[0] - 1, false));
      dth = th - theta(this->template coord<1>(pos[1] - 1, false));
      dph = ph - phi(this->template coord<2>(pos[2] - 1, false));

      // TODO: Something more intelligent might need to be done at these
      // edge cases, but for now we anticipate never having a guard cell
      // right at the poles in 3D, so we can just use the same formula
      // as for the interior cells.
      if (pos[1] == this->guard[1] && th_s < 0.1 * this->delta[1]) {
        m_Ad[0][idx] = sqrt_gamma(a, r, th_s) * dth * dph;
      } else if (pos[1] == this->dims[1] - this->guard[1] &&
                 math::abs(th_s - M_PI) < 0.1 * this->delta[1]) {
        m_Ad[0][idx] = sqrt_gamma(a, r, th_s) * dth * dph;
      } else {
      ...
      m_Ad[1][idx] = sqrt_gamma(a, r_s, th) * dr * dph;
      if (m_Ad[1][idx] != m_Ad[1][idx]) {
        Logger::print_err("m_Ad1 at ({}, {}) is NaN!", pos[0], pos[1]);
      }

      if (pos[1] == this->guard[1] && th_s < 0.1 * this->delta[1]) {
        m_Ad[2][idx] = sqrt_gamma(a, r_s, th_s) * dr * dth;
      } else if (pos[1] == this->dims[1] - this->guard[1] &&
                 math::abs(th_s - M_PI) < 0.1 * this->delta[1]) {
        m_Ad[2][idx] = sqrt_gamma(a, r_s, th_s) * dr * dth;
      } else {
    ...
    if constexpr (Conf::dim == 2) {
      ...
      m_ag13dr_b[idx] =
          gauss_quad([this, th](auto x) { return ag_13(a, x, th); }, r_s,
                     radius(this->template coord<0>(pos[0] + 1, true)));
      ...
      m_gbetadth_b[idx] =
          gauss_quad([this, r](auto x) { return sq_gamma_beta(a, r, x); }, th_s,
                     theta(this->template coord<1>(pos[1] + 1, true)));
