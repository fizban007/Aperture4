...
int main(int argc, char *argv[]) {
  ...
  env.params().add("damping_boundary", std::vector<bool>({false, true, false, false}));
  ...
  auto boundary = env.register_system<boundary_condition<Conf>>();
  auto solver = env.register_system<field_solver<Conf, exec_policy_dynamic,
                                                 coord_policy_cartesian>>(grid, &comm);
