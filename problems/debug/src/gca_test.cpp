...
template class radiative_transfer<Config<3>, exec_policy_dynamic,
...
main(int argc, char *argv[]) {
  ...
  domain_comm<Conf, exec_policy_dynamic> comm(&argc, &argv);
  ...
  auto ptc_data = env.register_data<particle_data_t>(
      ...
      exec_policy_dynamic<Conf>::debug_mem_type());
  auto ph_data = env.register_data<photon_data_t>(
      "photons", env.params().get_as("max_ph_num", 1000000l),
      exec_policy_dynamic<Conf>::debug_mem_type());

  auto pusher = env.register_system<
      ptc_updater<Conf, exec_policy_dynamic, coord_policy_cartesian_gca_lite>>(
      grid, &comm);
  auto rad = env.register_system<
      radiative_transfer<Conf, exec_policy_dynamic, coord_policy_cartesian,
  ...
  ptc_append(typename exec_policy_dynamic<Conf>::exec_tag{}, *ptc, rel_x,
             {ptc_p_parallel, 0.0, 0.0}, cell, 1.0,
