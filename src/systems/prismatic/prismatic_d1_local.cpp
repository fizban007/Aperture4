...
prismatic_d1_local prismatic_d1_local::build(
  ...
  out.d1_tri_h.row_ptr.assign(L_tri.owned_size() + 1, 0);
  for (int l = 0; l < L_tri.owned_size(); ++l) {
    ...
    for (int j = row_start; j < row_end; ++j) {
      ...
      out.d1_tri_h.col_idx.push_back(local_he);
      out.d1_tri_h.val.push_back(d1_val[j]);
    }
    out.d1_tri_h.row_ptr[l + 1] = int(out.d1_tri_h.col_idx.size());
  }

  // ===== d1: rect faces → h and v edges =====
  out.d1_rect_h.row_ptr.assign(L_rect.owned_size() + 1, 0);
  out.d1_rect_v.row_ptr.assign(L_rect.owned_size() + 1, 0);
  for (int l = 0; l < L_rect.owned_size(); ++l) {
    ...
    for (int j = row_start; j < row_end; ++j) {
      ...
      if (g_edge < N_h_edges) {
        ...
        out.d1_rect_h.col_idx.push_back(local_he);
        out.d1_rect_h.val.push_back(d1_val[j]);
      } else {
        const int local_ve = L_ve.to_local(g_edge - N_h_edges);
        assert(local_ve >= 0 && "v_edge boundary of owned rect must be in halo");
        out.d1_rect_v.col_idx.push_back(local_ve);
        out.d1_rect_v.val.push_back(d1_val[j]);
      }
    }
    out.d1_rect_h.row_ptr[l + 1] = int(out.d1_rect_h.col_idx.size());
    out.d1_rect_v.row_ptr[l + 1] = int(out.d1_rect_v.col_idx.size());
  }

  // ===== d1^T: h edges → tri and rect faces =====
  out.d1t_h_tri.row_ptr.assign(L_he.owned_size() + 1, 0);
  out.d1t_h_rect.row_ptr.assign(L_he.owned_size() + 1, 0);
  for (int l = 0; l < L_he.owned_size(); ++l) {
    ...
    for (int j = row_start; j < row_end; ++j) {
      ...
      if (g_face < N_tri_faces) {
        ...
        out.d1t_h_tri.col_idx.push_back(local_tri);
        out.d1t_h_tri.val.push_back(d1t_val[j]);
      } else {
        const int local_rect = L_rect.to_local(g_face - N_tri_faces);
        assert(local_rect >= 0 &&
               "rect face adjacent to owned h_edge must be in halo");
        out.d1t_h_rect.col_idx.push_back(local_rect);
        out.d1t_h_rect.val.push_back(d1t_val[j]);
      }
    }
    out.d1t_h_tri.row_ptr[l + 1]  = int(out.d1t_h_tri.col_idx.size());
    out.d1t_h_rect.row_ptr[l + 1] = int(out.d1t_h_rect.col_idx.size());
  }

  // ===== d1^T: v edges → rect faces =====
  out.d1t_v_rect.row_ptr.assign(L_ve.owned_size() + 1, 0);
  for (int l = 0; l < L_ve.owned_size(); ++l) {
    ...
    for (int j = row_start; j < row_end; ++j) {
      ...
      out.d1t_v_rect.col_idx.push_back(local_rect);
      out.d1t_v_rect.val.push_back(d1t_val[j]);
    }
    out.d1t_v_rect.row_ptr[l + 1] = int(out.d1t_v_rect.col_idx.size());
  }
