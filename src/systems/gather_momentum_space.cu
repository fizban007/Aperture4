...
//   kernel_launch(
//       [num, num_bins, lower, upper] __device__(auto ptc, auto e_p1, auto
//       e_p2,
//                                                auto e_p3, auto p_p1, auto
//                                                p_p2, auto p_p3, int
//                                                downsample) {
...
//             index_t<Conf::dim + 1> pos_out(0, (pos - grid.guards()) /
//             downsample);
...
//             (num_bins[0] - 1)); int bin2 = floor((p2 - lower[1]) / (upper[1]
//             - lower[1]) * (num_bins[1] - 1)); int bin3 = floor((p3 -
//             lower[2]) / (upper[2] - lower[2]) * (num_bins[2] - 1));

//             if (sp == (int)PtcType::electron) {
//               pos_out[0] = bin1;
//               atomicAdd(&e_p1[idx_t(pos_out, extent_t<Conf::dim +
//               1>(num_bins[0], ext_out))],
//                         weight);
//               pos_out[0] = bin2;
//               atomicAdd(&e_p2[idx_t(pos_out, extent_t<Conf::dim +
//               1>(num_bins[1], ext_out))],
//                         weight);
//               pos_out[0] = bin3;
//               atomicAdd(&e_p3[idx_t(pos_out, extent_t<Conf::dim +
//               1>(num_bins[2], ext_out))],
//                         weight);
//             } else if (sp == (int)PtcType::positron) {
//               pos_out[0] = bin1;
//               atomicAdd(&p_p1[idx_t(pos_out, extent_t<Conf::dim +
//               1>(num_bins[0], ext_out))],
//                         weight);
//               pos_out[0] = bin2;
//               atomicAdd(&p_p2[idx_t(pos_out, extent_t<Conf::dim +
//               1>(num_bins[1], ext_out))],
//                         weight);
//               pos_out[0] = bin3;
//               atomicAdd(&p_p3[idx_t(pos_out, extent_t<Conf::dim +
//               1>(num_bins[2], ext_out))],
//                         weight);
//             }
//           }
